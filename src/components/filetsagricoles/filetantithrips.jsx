...
export default function Filetantithrips({product}) {
  const { addToCart } = useCart()
  const [addedToCart, setAddedToCart] = useState({})

  const [filetther, setFiletther] = useState([])
  ...
  useEffect(() => {
    const fetchCorgettes = async () => {
      try {
        const response = await axios.get("http://localhost:8001/products?category=FILETANTITHRIPS")
        setFiletther(response.data)
        setLoading(false)
      } catch (error) {
  ...
  const deletegranules = (id) => {
    if (window.confirm("Voulez-vous vraiment supprimer cet filet ?")) {
      axios
        ...
        .then(() => {
          setFiletther((prev) => prev.filter((tomate) => tomate.id !== id))
        })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:8001/products", {
        ...newSobule,
        category: "FILETANTITHRIPS",
      })
      setFiletther((prev) => [...prev, response.data])
      setnewSobule({
        image: "",
        nom: "",
        description: "",
  ...
  return (
    ...
    <div className="engrais-container">
      <div className="engrais-header">
        <h2 className="engrais-title">FILET ANTI-THRIPS</h2>
        {stateuser ? (
      <button
        style={{
          width: "350px",
