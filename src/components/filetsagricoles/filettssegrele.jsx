...
export default function Filettessgrele({product}) {
  ...
  useEffect(() => {
    const fetchCorgettes = async () => {
      try {
        const response = await axios.get("http://localhost:8001/products?category=FILET TISSE ANTI GRÊLE")
        setFiletgrel(response.data)
        setLoading(false)
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:8001/products", {
        ...newSobule,
        category: "FILET TISSE ANTI GRÊLE",
      })
      setFiletgrel((prev) => [...prev, response.data])
      setnewSobule({
        image: "",
        nom: "",
        description: "",
