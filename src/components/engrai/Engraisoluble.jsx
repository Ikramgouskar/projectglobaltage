...
export default function Engraisoluble({product}) {
  ...
  useEffect(() => {
    const fetchCorgettes = async () => {
      try {
        const response = await axios.get("http://localhost:8000/products?category=SOLUBLES")
        setEgranules(response.data)
        setLoading(false)
      } catch (error) {
  ...
  const deletegranules = (id) => {
    if (window.confirm("Voulez-vous vraiment supprimer cet Engrais ?")) {
      axios
        ...
        .then(() => {
          setEgranules((prev) => prev.filter((tomate) => tomate.id !== id))
        })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setEgranules((prev) => [...prev, response.data])
      setnewSobule({
        image: "",
        nom: "",
        description: "",
  ...
  useEffect(() => {
    ...
    const hardcodedImages = [
      "calciamnitrate.png",
      "chloruredepotasse.png",
      "kaliop.png",
      "map oluble.png",
      "mkp.png",
      "nitratedecalcumfertigrade.png",
      "nitratedepota.png",
      "soluamspremium.png",
    ]
  ...
  return (
    ...
    <div className="engrais-container">
      <div className="engrais-header">
        <h2 className="engrais-title">NPK Granules</h2>
        {stateuser ? (
      <button
        style={{
          width: "350px",
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          >
            ...
            {images.map((image, index) => (
              <option key={index} value={`/imagedengrain/imgsoluble/${image}`}>
                {image}
              </option>
            ))}
          </select>
