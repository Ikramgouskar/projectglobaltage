...
export default function Boitsti({product}) {
    const { addToCart } = useCart()
    const [addedToCart, setAddedToCart] = useState({})
  
  const [boitsti, setBoitsti] = useState([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)
  const [stateuser, setStateuser] = useState(null);  const [showAddModal, setShowAddModal] = useState(false)
  ...
  useEffect(() => {
    const fetchCorgettes = async () => {
      try {
        const response = await axios.get("http://localhost:8002/products?category=BIOSTIMULANT")
        setBoitsti(response.data)
        setLoading(false)
      } catch (error) {
  ...
  const deleteENGRAI = (id) => {
    if (window.confirm("Voulez-vous vraiment supprimer cette VEGETABLE ?")) {
      axios
        ...
        .then(() => {
          setBoitsti((prev) => prev.filter((tomate) => tomate.id !== id))
        })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setBoitsti((prev) => [...prev, response.data])
      setnewSobule({
        image: "",
        nom: "",
        description: "",
  ...
  return (
    ...
    <div className="engrais-container">
      <div className="engrais-header">
        <h2 className="engrais-title">BIOSTIMULANT</h2>
        {stateuser ? (
      <button
        style={{
          width: "400px",
