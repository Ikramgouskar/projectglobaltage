...
export default function OligoElementns() {
  ...
  useEffect(() => {
    const fetchCorgettes = async () => {
      try {
        const response = await axios.get("http://localhost:8002/products?category=OLIGO-ELEMENTS")
        setOligo(response.data)
        setLoading(false)
      } catch (error) {
  ...
  const deleteENGRAI = (id) => {
    if (window.confirm("Voulez-vous vraiment supprimer cette VEGETABLE ?")) {
      axios
        ...
        .then(() => {
          setOligo((prev) => prev.filter((tomate) => tomate.id !== id))
        })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setOligo((prev) => [...prev, response.data])
      setnewSobule({
        image: "",
        nom: "",
        description: "",
  ...
  return (
    ...
    <div className="engrais-container">
      <div className="engrais-header">
        <h2 className="engrais-title">OLIGO-ELEMENTS</h2>
        {stateuser ? (
      <button
        style={{
          width: "400px",
